...
class EditDDIMScheduler:
    ...
    def set_timesteps(self, num_inference_steps, offset=0, device=None):
        """set time steps."""
...
class AsyrpScheduler(EditDDIMScheduler):
    ...
    def step(
        ...
        use_clipped_model_output: bool = False,
        generator=None,
        model_output_orig: Optional[Union[torch.FloatTensor,
                                          np.ndarray]] = None,
    ):
        if model_output_orig is None and gradient is None:
            return super().step(model_output, timestep, sample, cond_fn,
                                cond_kwargs, eta, use_clipped_model_output,
                                generator)

        if gradient is not None:
            # gradient is passed and model_output_orig is None, means we want
            # use a specific guidance but not asyrp
            # therefore set model_output_orig as model_output
            model_output_orig = model_output \
                if model_output_orig is None else model_output_orig
        # NOTE: perform asyrp
        output = {}
        ...
        # 3. compute predicted original sample from predicted noise also called
        # "predicted x_0" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf # noqa
        # NOTE: model_output -> model_output_orig (for asyrp)
        pred_original_sample = (sample - (
            (beta_prod_t)**(0.5)) * model_output) / alpha_prod_t**(0.5)
        if pred is not None:
            pred_original_sample = pred
        ...
        if gradient is not None:
            do_guidance = True
            assert cond_fn is None, ('cond_fn and gradient should not be passed '
                                     'at the same time.')
        ...
        if cond_fn is not None:
            ...
            if cond_fn.__name__ == 'classifier_grad':
                ...
                classifier = cond_kwargs['classifier']
                classifier_scale = cond_kwargs['classifier_scale']
                gradient = cond_fn(classifier,
                                   sample,
                                   timestep,
                                   y=y,
                                   classifier_scale=classifier_scale)
            else:
                gradient = cond_fn(cond_kwargs.pop('unet'), self, sample,
                                   timestep, beta_prod_t,
        ...
        if do_guidance:
            ...
            pred_original_sample = (
                sample -
                (beta_prod_t**(0.5)) * model_output) / alpha_prod_t**(0.5)
            output['guidance'] = - (beta_prod_t**0.5) * gradient
        ...
        # 6. compute "direction pointing to x_t" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf # noqa
        pred_sample_direction = (1 - alpha_prod_t_prev -
                                 std_dev_t**2)**(0.5) * model_output_orig
